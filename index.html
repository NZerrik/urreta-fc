...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .clima-badge .clima-icon { font-size: 1rem; }
  .clima-badge .clima-temp { font-family: 'Bebas Neue', sans-serif; font-size: 1rem; color: var(--blanco); }
  .clima-badge .clima-lluvia { color: #60a5fa; font-size: 0.72rem; }
  ...
  .clima-proximo-item .ci-icon { font-size: 1.4rem; }
  .clima-proximo-item .ci-valor { font-family: 'Bebas Neue', sans-serif; font-size: 1.1rem; color: var(--blanco); }
  .clima-proximo-item .ci-label { font-size: 0.62rem; color: #666; text-transform: uppercase; letter-spacing: 1px; }

  .clima-loading { color: #555; font-size: 0.75rem; font-style: italic; }
  ...
  .status-suspendido { background: rgba(239,68,68,0.2); color: var(--rojo-gol); }
...
<script>
// ====== FECHAS ======
// F1: 16/03/2026 → W/O contra Bochas
// F2: 22/03/2026 → Urreta 7-1 Malvín Alto
// F3: 29/03/2026 → E. del Norte vs Urreta
// Domingo 05/04: no hay fútbol
// F4: 12/04/2026
// F5 en adelante: cada domingo desde 19/04

const FECHAS = [
  '16/03/2026', // F1 - Bochas W/O
  '22/03/2026', // F2 - Malvín Alto
  '29/03/2026', // F3 - E. del Norte
  '12/04/2026', // F4
  '19/04/2026', // F5
  '26/04/2026', // F6
  '03/05/2026', // F7
...
const FIXTURE = [
  ...
  { rival: 'E. del Norte',  esLocal: false, fecha_txt: 'A confirmar', suspendido: true }, // F3 - suspendida, fecha a confirmar
...
// Hora de cada partido. null = sin confirmar
const HORAS_PARTIDO = {
  1:  null,  // W/O
  2:  14,    // 14:00hs
  3:  16,    // 16:00hs
...
function getHoraPartido(fechaNum) {
  const h = HORAS_PARTIDO[fechaNum];
  return h !== null && h !== undefined ? `${h}:00hs` : null;
}
...
function isProximo(idx) {
  ...
  // Si está suspendido y sin resultado, es el próximo
  if(partido.suspendido) return true;
  const hoy = new Date();
...
function getCountdownHtml(fecha_txt, hora) {
  if (!fecha_txt || fecha_txt === 'A confirmar') {
    return `<div class="countdown-grid">
      <div class="countdown-block"><span class="countdown-num">00</span><span class="countdown-label">días</span></div>
      <div class="countdown-block"><span class="countdown-num">00</span><span class="countdown-label">horas</span></div>
      <div class="countdown-block"><span class="countdown-num">00</span><span class="countdown-label">min</span></div>
      <div class="countdown-block"><span class="countdown-num">00</span><span class="countdown-label">seg</span></div>
    </div>`;
  }
  const [dd, mm, yy] = fecha_txt.split('/').map(Number);
  const h = hora ?? 10;
  const target = new Date(yy, mm - 1, dd, h, 0, 0);
  const now = new Date();
...
function tickCountdown(fecha_txt, hora) {
  const el = document.getElementById('countdownLive');
  if (!el) { clearInterval(countdownInterval); return; }
  el.innerHTML = getCountdownHtml(fecha_txt, hora);
}
...
function renderProximo() {
  ...
  const horaRaw = HORAS_PARTIDO[proxIdx + 1];
  const horaStr = horaRaw !== null && horaRaw !== undefined ? `${horaRaw}:00` : null;
  ...
  if (p.suspendido) {
    document.getElementById('countdownLive').innerHTML = `<div style="font-family:'Bebas Neue',sans-serif;font-size:1.3rem;color:#EF4444;letter-spacing:3px;text-align:center;margin-top:8px;">⚠️ SUSPENDIDO — FECHA A CONFIRMAR</div>`;
    clearInterval(countdownInterval);
  } else {
    document.getElementById('countdownLive').innerHTML = getCountdownHtml(p.fecha_txt, horaRaw);
    clearInterval(countdownInterval);
    countdownInterval = setInterval(() => tickCountdown(p.fecha_txt, horaRaw), 1000);
  }

  document.getElementById('countdownSubtitulo').innerHTML =
    p.suspendido
      ? `Urreta vs <span>${p.rival}</span> · Fecha suspendida · Nueva fecha a definir`
      : `Urreta vs <span>${p.rival}</span> · ${p.fecha_txt === 'A confirmar' ? '' : 'Domingo '}${p.fecha_txt}${horaStr ? ' · ' + horaStr + 'hs' : ' · Hora a confirmar'}`;
  ...
  <div class="proximo-card fade-in">
    ...
    <div class="proximo-meta">
      <div class="meta-item">
        ...
        <span>${p.fecha_txt === 'A confirmar' ? 'Fecha' : 'Domingo'} ${p.fecha_txt} · Fecha ${proxIdx+1}${horaStr ? ' · ' + horaStr + 'hs' : ' · Hora a confirmar'}</span>
      </div>
      <div class="meta-item">
        <span class="icon">📍</span>
    ...
    <div id="clima-proximo" class="clima-loading">Cargando clima...</div>
  ...
  // Cargar clima del próximo partido (solo si tiene fecha)
  if (!p.suspendido) injectClima('clima-proximo', p.fecha_txt, 'proximo');
}
...
function renderFixture() {
  ...
  FIXTURE.forEach((p, idx) => {
    ...
    const statusLabel = {
      'jugado': 'Jugado', 'proximo': p.suspendido ? 'Suspendido' : 'Próximo', 'pendiente': 'Pendiente', 'libre': 'Fecha libre'
    }[status];
    const statusClass = p.suspendido ? 'status-suspendido' : `status-${status}`;
    ...
    const climaId = `clima-fecha-${fechaNum}`;
    const horaFecha = HORAS_PARTIDO[fechaNum];
    const horaLabel = (!p.suspendido && horaFecha) ? `· ${horaFecha}:00hs` : '';
    card.innerHTML = `
      <div class="fecha-header">
        <span class="fecha-num">Fecha ${fechaNum}</span>
        <span class="fecha-date">${p.fecha_txt || '—'} ${horaLabel}</span>
        <span id="${climaId}"></span>
        <span class="fecha-status ${statusClass}">${statusLabel}</span>
      </div>
      ${bodyHtml}`;
    grid.appendChild(card);

    // Cargar clima async para esta fecha (solo si no es libre y tiene fecha)
    if (!p.libre && p.fecha_txt) {
      injectClima(climaId, p.fecha_txt, 'badge');
    }
  });
}
...
async function fetchClima(fechaStr) {
  if (_climaCache[fechaStr]) return _climaCache[fechaStr];
  try {
    const [dd, mm, yyyy] = fechaStr.split('/');
    const fecha = `${yyyy}-${mm}-${dd}`;
    const hoy = new Date();
    const fechaDate = new Date(parseInt(yyyy), parseInt(mm)-1, parseInt(dd));
    // Usar API de histórico para fechas pasadas, forecast para futuras
    const esPasado = fechaDate < hoy;
    const baseUrl = esPasado
      ? 'https://archive-api.open-meteo.com/v1/archive'
      : 'https://api.open-meteo.com/v1/forecast';
    const url = `${baseUrl}?latitude=${MVD_LAT}&longitude=${MVD_LON}&daily=weathercode,temperature_2m_max,temperature_2m_min,precipitation_probability_max,windspeed_10m_max&timezone=America%2FMontevideo&start_date=${fecha}&end_date=${fecha}`;
    const r = await fetch(url);
    const data = await r.json();
    if (!data.daily) return null;
    const clima = {
      icon:   wmoToEmoji(data.daily.weathercode[0]),
      ...
      viento: Math.round(data.daily.windspeed_10m_max[0]),
    };
    _climaCache[fechaStr] = clima;
    return clima;
  } catch(e) { return null; }
}

function climaBadgeHtml(clima) {
  if (!clima) return '';
  return `<div class="clima-badge">
    <span class="clima-icon">${clima.icon}</span>
    <span class="clima-temp">${clima.tmax}°</span>
    <span class="clima-lluvia">💧${clima.lluvia}%</span>
  </div>`;
}

function climaProximoHtml(clima) {
  if (!clima) return '';
  return `<div class="clima-proximo">
    <div class="clima-proximo-item">
      <span class="ci-icon">${clima.icon}</span>
      <span class="ci-valor">${clima.tmax}°/${clima.tmin}°</span>
      <span class="ci-label">Temperatura</span>
    </div>
    <div class="clima-proximo-item">
      <span class="ci-icon">💧</span>
      <span class="ci-valor">${clima.lluvia}%</span>
      <span class="ci-label">Lluvia</span>
    </div>
    <div class="clima-proximo-item">
      <span class="ci-icon">💨</span>
      <span class="ci-valor">${clima.viento}</span>
      <span class="ci-label">km/h viento</span>
    </div>
  </div>`;
}

// Inyectar clima en un elemento por id
async function injectClima(elementId, fecha_txt, modo) {
  const el = document.getElementById(elementId);
  if (!el) return;
  const clima = await fetchClima(fecha_txt);
  el.innerHTML = modo === 'proximo' ? climaProximoHtml(clima) : climaBadgeHtml(clima);
}

// Arrancar la app — primero renderiza con datos locales,
// luego intenta cargar resultados.json del servidor y re-renderiza
renderAll();

fetch('./resultados.json?t=' + Date.now())
  .then(r => r.json())
  .then(data => {
    if (data.urreta) Object.assign(resultados, data.urreta);
    if (data.fechas) {
      Object.keys(data.fechas).forEach(fechaKey => {
        const idx = parseInt(fechaKey) - 1;
        if (FIXTURE_FECHA[idx]) {
          data.fechas[fechaKey].forEach(p => {
            const row = FIXTURE_FECHA[idx].find(r => r.local === p.local && r.visitante === p.visitante);
            if (row && p.gl !== null && p.gl !== undefined) { row.gl = p.gl; row.gv = p.gv; if (p.wo) row.wo = true; }
          });
        }
      });
    }
    if (data.ultima_actualizacion) {
      const d = new Date(data.ultima_actualizacion);
      const el = document.getElementById('ultimaActualizacion');
      if (el) el.textContent = `Actualizado: ${d.toLocaleDateString('es-UY')} ${d.toLocaleTimeString('es-UY',{hour:'2-digit',minute:'2-digit'})}hs`;
    }
    renderAll();
  })
  .catch(() => { /* archivo local o sin conexión — ya renderizó arriba */ });
